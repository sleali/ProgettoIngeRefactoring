...
public class Main_simula_PN {
    public static void simula(ControllerRepositoryPN repo)
    {
        if(repo.sizeFileNames() == 0)
            System.out.println("Prima di simulare una rete PN e' necessario averne inserita almeno una\n");
        else {
            System.out.println(repo.toStringFileNames());
            int retePN = (InputDati.leggiIntero("Selezionare la rete PN che si vuole simulare: ", 1, repo.sizeFileNames()) -1);
            SimulaPN sim = new SimulaPN(repo.getRete(retePN));
            simulation(sim);
        }
    }

    private static void simulation(SimulaPN sim)
    {
        int prosegui=1;
        ArrayList<TransizioneN> abil;
        ViewRetePN view = new ViewRetePN(sim.getRete());
    ...
    private static TransizioneN scegliTransizione(ArrayList<TransizioneN> list){
        System.out.println("Scegli una transizione tra quelle proposte qui sotto:\n\n");
        for(int i=0 ; i<list.size() ; i++) {
            ViewTransizioneN view = new ViewTransizioneN(list.get(i));
